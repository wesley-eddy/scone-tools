...
SCONE_PORT=60000
PLAIN_UDP6_PORT=10001
NON_SCONE6_PORT=20001
SCONE6_PORT=60001
...
ip netns exec $SCONE_CAP_NS nc -u -l -6 $((PLAIN_UDP6_PORT + 1)) & nc6_pid=$!
ip netns exec $SCONE_CAP_NS ./hq --mode=server --host=$CAP_IPV6 --port=$NON_SCONE6_PORT --logtostderr=false & hqs1_ipv6_pid=$!
ip netns exec $SCONE_CAP_NS ./hq --mode=server --host=$CAP_IPV6 --port=$SCONE6_PORT --logtostderr=false --h2port=7777 & hqs2_ipv6_pid=$!
...
ip netns exec $SCONE_USER_NS ./hq --mode=client --host=$CAP_IPV6 --port=$NON_SCONE6_PORT --path=/
sleep 2
echo "  Doing SCONE-enabled fetch (IPv6)."
ip netns exec $SCONE_USER_NS ./hq --mode=client --host=$CAP_IPV6 --port=$SCONE6_PORT --path=/
sleep 2
